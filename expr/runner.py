...
for pp in args.prefetcher:
    ...
    print(f"\n{'='*60}\n")
    print(f"> Executable: {CYAN}../bin/champsim.{pp}{END}")
    print(f"> Warm-up: {CYAN}{WARM_UP}{END}, Interval: {CYAN}{INTERVAL}{END}")
    
    os.system(f"stat ../bin/champsim.{pp} | grep 最近更改")
    if os.path.exists(f"../bin/champsim_config_champsim.{pp}.json"):
        with open(f"../bin/champsim_config_champsim.{pp}.json", "r", encoding="utf-8") as f:
            data = json.load(f)
    ...
    if f"champsim.{pp}" not in os.listdir("../bin/"):
        print(f"> {RED}Error: Executable champsim.{pp} not found in ../bin/. Please compile first with --compile flag.{END}")
        sys.exit(1)
    
def launch_task(p, w):
    mode_is_mc = p.endswith(".mc")
    
    work = [f"/mnt/data/lyq/Kairos/bin/champsim.{p}", "--warmup-instructions", f"{WARM_UP}", "--simulation-instructions", f"{INTERVAL}", trace_path_map[w]]
    name = f"{w}.log"
    if mode_is_mc:
    ...
    with open(f"{out_path}/{name}", "w") as f:
        ...
        data = None
        if os.path.exists(f"../bin/champsim_config_champsim.{pp}.json"):
            with open(f"../bin/champsim_config_champsim.{pp}.json", "r", encoding="utf-8") as rf:
                data = json.load(rf)
        if data is not None:
