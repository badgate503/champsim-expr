...
struct filtetpMetaTableEntry {

  uint64_t correlated_addr;
  bool used;

  filtetpMetaTableEntry() : correlated_addr(0), used(false) {};
  filtetpMetaTableEntry(uint64_t addr) : correlated_addr(addr) {};
};

class filtetpMetaTable : public LRUSetAssociativeCache<filtetpMetaTableEntry>
{
  typedef LRUSetAssociativeCache<filtetpMetaTableEntry> Super;

public:
  std::unordered_map<uint64_t, std::set<uint64_t>> reverse_metatable;
  filtetp* prefetcher;

  filtetpMetaTable(int size, int num_ways) : Super(size, num_ways), prefetcher(nullptr) {}

  void setpp(filtetp* p) { prefetcher = p; }

  filtetpMetaTableEntry* find(uint64_t key)
  {
  ...
  */
  bool insert(uint64_t key, const filtetpMetaTableEntry& data);

  Entry* erase(uint64_t key) { return Super::erase(key); }
};

class filtetp : public champsim::modules::prefetcher
{
public:
  // BaseTags* cachetags;
  ...
  struct PCEntry {
    ...
    uint64_t total_cnt;
    bool pass;
    PCEntry() : miss_cnt(6), total_cnt(24), pass(false) {}
  };
  std::map<uint64_t, PCEntry> hit_hist; // pc->hit history
  ...
  int issue_metatable(filtetpMetaTable* metaTable, uint64_t lookup, std::vector<uint64_t>& addresses);
...
#endif // __MEM_CACHE_PREFETCH_filtetp_HH__
