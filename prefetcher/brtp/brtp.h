...
struct brtpMetaTableEntry {

  uint64_t correlated_addr;
  bool used;

  brtpMetaTableEntry() : correlated_addr(0), used(false) {};
  brtpMetaTableEntry(uint64_t addr) : correlated_addr(addr) {};
};

class brtpMetaTable : public BRRIPSetAssociativeCache<brtpMetaTableEntry>
{
  typedef BRRIPSetAssociativeCache<brtpMetaTableEntry> Super;

public:
  std::unordered_map<uint64_t, std::set<uint64_t>> reverse_metatable;
  brtp* prefetcher;

  brtpMetaTable(int size, int num_ways) : Super(size, num_ways), prefetcher(nullptr) {}

  void setpp(brtp* p) { prefetcher = p; }

  brtpMetaTableEntry* find(uint64_t key)
  {
  ...
  */
  bool insert(uint64_t key, const brtpMetaTableEntry& data);

  Entry* erase(uint64_t key) { return Super::erase(key); }
};

class brtp : public champsim::modules::prefetcher
{
public:
  // BaseTags* cachetags;
  ...
  brtpMetaTable* metaTable = new brtpMetaTable(META_TABLE_SIZE, META_TABLE_ASSOC);
  ...
  int issue_metatable(brtpMetaTable* metaTable, uint64_t lookup, std::vector<uint64_t>& addresses);
...
#endif // __MEM_CACHE_PREFETCH_brtp_HH__
