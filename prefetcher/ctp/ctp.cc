...
void ctp::invoke_prefetcher(uint64_t ip, uint64_t addr, uint8_t cache_hit, uint8_t type, vector<uint64_t>& pref_addr)
{
  if (disablePF)
    return;
  if (ip == 0) {
  ...
  ctpMetaTableEntry* metadata = metaTable->find(block_addr);
  ctpMRBTableEntry* reuseData = mrbTable->find(block_addr);

  uint64_t lastAddr = pcTable.find(ip) != pcTable.end() ? pcTable[ip].lastAddr : 0;
  uint64_t lastLastAddr = pcTable.find(ip) != pcTable.end() ? pcTable[ip].lastLastAddr : 0;
  ...
  uint64_t conflictTableKey = lastAddr << 32 | block_addr;
  if (conflictTable.find(conflictTableKey) != conflictTable.end()) {
    conflictTable[conflictTableKey].confident = true;
    pref_addr.push_back(conflictTable[conflictTableKey].targetAddr << LOG2_BLOCK_SIZE);
#ifdef ELABORATE_LOG
    logfile << std::dec << llc_cache->current_cycle() << " ISSUE CT " << std::hex << ip << " " << (conflictTableKey) << " " << (conflictTable[conflictTableKey].targetAddr) << std::endl;
#endif
  }
  // else
  {
    // baseline's origin code
  ...
  if (lastAddr != 0) {
    ctpMetaTableEntry* lastMeta = metaTable->find(lastAddr);
    if (lastMeta) {
      bool matched = false;
      ...
      if (!matched) {
        // ctp1.0
        // conflictTableKey = lastLastAddr << 32 | lastAddr;
        // conflictTable[conflictTableKey] = conflictTableEntry(block_addr);

        // ctp2.0
        // conflictTableKey = lastMeta->lastAddr << 32 | lastAddr;
        // conflictTable[conflictTableKey] = conflictTableEntry(lastMeta->correlatedAddr);
        // ctpMetaTableEntry temp_entry(block_addr, lastLastAddr);
      ...
      //   ctpMetaTableEntry temp_entry(block_addr);
      //   metaTable->insert(lastAddr, temp_entry, 1);

      //   // victim buffer logic
      //   if (profileReplTable[ip] > 1) {
      //     ctpMRBTableEntry* victimMeta = mrbTable->find(lastAddr);
      //     if (!victimMeta) {
      //       ctpMRBTableEntry temp_entry(victimAddr);
      ...
      ctpMetaTableEntry temp_entry(block_addr, lastAddr);
  ...
  pcTable[ip].lastLastAddr = pcTable[ip].lastAddr;
  pcTable[ip].lastAddr = block_addr;
}

int ctp::issue_metatable(ctpMetaTable* metaTable, uint64_t lookup, uint64_t pc, std::vector<uint64_t>& addresses)
{
  int issued = 0;
  for (int i = 0; i < globalDegree; i++) {
    ctpMetaTableEntry* candidate = metaTable->find(lookup);
...
int ctp::issue_mrbtable(ctpMRBTable* mrbTable, uint64_t lookup, uint64_t pc, std::vector<uint64_t>& addresses)
{
  std::cout << "deprecated,Should not issue" << std::endl;
  int issued = 0;
  for (int i = 0; i < globalDegree; i++) {
    ctpMRBTableEntry* candidate = mrbTable->find(lookup);
    if (candidate == nullptr)
      break;
    addToUsedPool(lookup);
    if (candidate->correlatedAddr != 0) {
...
void ctp::outPrefetcherPGOInfo() {}

uint32_t ctp::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type,
...
uint32_t ctp::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
  return metadata_in;
}

void ctp::prefetcher_final_stats()
{
#ifdef ELABORATE_LOG
  logfile.close();
#endif
...
void ctp::prefetcher_late_prefetch(champsim::address addr, champsim::address ip, std::string where)
{
#ifdef ELABORATE_LOG
...
void ctp::prefetcher_cycle_operate() {}

bool ctpMetaTable::insert(uint64_t key, const ctpMetaTableEntry& data, uint8_t priority)
{
