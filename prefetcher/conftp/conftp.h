...
// #define INF_CONFLICT_TABLE
#define CONFLICT_TABLE_SIZE 4096 * 1 * 12
#define CONFLICT_TABLE_ASSOC 12
...
struct conftpMetaTableEntry {

  uint64_t correlated_addr;
  bool used;

  conftpMetaTableEntry() : correlated_addr(0), used(false) {};
  conftpMetaTableEntry(uint64_t addr) : correlated_addr(addr) {};
};

class conftpMetaTable : public LRUSetAssociativeCache<conftpMetaTableEntry>
{
  typedef LRUSetAssociativeCache<conftpMetaTableEntry> Super;

public:
  std::unordered_map<uint64_t, std::set<uint64_t>> reverse_metatable;
  conftp* prefetcher;

  conftpMetaTable(int size, int num_ways) : Super(size, num_ways), prefetcher(nullptr) {}

  void setpp(conftp* p) { prefetcher = p; }

  conftpMetaTableEntry* find(uint64_t key)
  {
  ...
  */
  bool insert(uint64_t key, const conftpMetaTableEntry& data);

  Entry* erase(uint64_t key) { return Super::erase(key); }
};

class conftp : public champsim::modules::prefetcher
{
public:
  // BaseTags* cachetags;
  ...
  conftpMetaTable* metaTable = new conftpMetaTable(META_TABLE_SIZE, META_TABLE_ASSOC);
  ...
  std::map<uint64_t, uint64_t> conflictTable;
#else
  SRRIPSetAssociativeCache<uint64_t>* conflictTable = new SRRIPSetAssociativeCache<uint64_t>(CONFLICT_TABLE_SIZE, CONFLICT_TABLE_ASSOC);
#endif
  // stat
  uint64_t conflict_table_lookups = 0;
  uint64_t conflict_table_hits = 0;
  uint64_t conflict_table_late_prefetches = 0;
  uint64_t conflict_table_useful_prefetches = 0;
  uint64_t conflict_table_useless_prefetches = 0;
  std::set<uint64_t> conflict_table_issued_prefetches, conflict_table_filled_prefetches;
  ...
  int issue_metatable(conftpMetaTable* metaTable, uint64_t lookup, std::vector<uint64_t>& addresses);
...
#endif // __MEM_CACHE_PREFETCH_conftp_HH__
