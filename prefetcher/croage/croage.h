...
public:
  ...
  // 移动构造函数
  LRUCache(LRUCache&& other) noexcept : count(other.count), capacity(other.capacity), head(other.head), tail(other.tail), cache(std::move(other.cache))
  {
    // 重置other的状态
    other.head = nullptr;
    other.tail = nullptr;
...
public:
  ...
  // 移动构造函数
  FIFOCache(FIFOCache&& other) noexcept : count(other.count), capacity(other.capacity), head(other.head), tail(other.tail), cache(std::move(other.cache))
  {
    // 重置other的状态
    other.head = nullptr;
    other.tail = nullptr;
...
public:
  // 构造函数
  CroageRepl(int capacity) : count(0), capacity(capacity), head(new DLinkedNode()), tail(new DLinkedNode())
  {
    head->pre = nullptr;
    tail->post = nullptr;
    head->post = tail;
  ...
  CroageRepl(const CroageRepl& other)
      : count(0) // 将在添加节点时递增
        ,
        capacity(other.capacity), head(new DLinkedNode()), tail(new DLinkedNode())
  {
    head->pre = nullptr;
    tail->post = nullptr;
    head->post = tail;
  ...
  // 移动构造函数
  CroageRepl(CroageRepl&& other) noexcept : count(other.count), capacity(other.capacity), cache(std::move(other.cache)), head(other.head), tail(other.tail)
  {
    other.count = 0;
    other.capacity = 0;
    other.head = nullptr;
...
public:
  ...
  public:
    ...
    std::pair<uint64_t, T> operator*() const { return {node->key, node->value}; }
  ...
  bool update(uint64_t pc, uint8_t cache_hit)
  {
    ...
    if (!entry) {
      ...
      if (entry->miss_count * 2 > entry->issue_count)
        return 1;
      if (entry->miss_count * 8 > total_miss)
        return 1;
    }
    if (total_miss > 511) {
      for (auto& i : entry_list) {
        if (i.miss_count * 8 > total_miss)
          key_pc.insert(i.pc);
  ...
  void resize_metadata(int8_t direction)
  {
    ...
    if (new_size != current_metadata_size) {
      ...
      if (llc_cache != nullptr) {
        ...
        if constexpr (champsim::debug_print) {
          std::cout << "[CROAGE] LLC partition updated: metadata_ways=" << occupied_ways << ", available_ways=" << available_ways << std::endl;
        }
      }
