...
void srtp::invoke_prefetcher(uint64_t ip, uint64_t addr, uint8_t cache_hit, uint8_t type, vector<uint64_t>& pref_addr)
{
  if (disablePF)
    return;
  if (ip == 0) {
  ...
  srtpMetaTableEntry* metadata = metaTable->find(block_addr);
  srtpMRBTableEntry* reuseData = mrbTable->find(block_addr);
  ...
  if (lastAddr != 0) {
    srtpMetaTableEntry* lastMeta = metaTable->find(lastAddr);
    if (lastMeta) {
      bool matched = false;
      ...
      if (!matched) {
        ...
        metaTable->insert(lastAddr, temp_entry);

        // victim buffer logic
        if (profileReplTable[ip] > 1) {
          srtpMRBTableEntry* victimMeta = mrbTable->find(lastAddr);
          if (!victimMeta) {
            srtpMRBTableEntry temp_entry(victimAddr);
      ...
      if (!inTraining && enablePGLRU && profileReplTable.find(ip) != profileReplTable.end())
        ...
        if (!metaTable->insert(lastAddr, temp_entry)) // lyq: profile中，prio = 0 ？
        {
          numEntriesinTable++;
        }
      }
...
int srtp::issue_metatable(srtpMetaTable* metaTable, uint64_t lookup, uint64_t pc, std::vector<uint64_t>& addresses)
{
  int issued = 0;
...
int srtp::issue_mrbtable(srtpMRBTable* mrbTable, uint64_t lookup, uint64_t pc, std::vector<uint64_t>& addresses)
{
  std::cout << "deprecated,Should not issue" << std::endl;
  int issued = 0;
  for (int i = 0; i < globalDegree; i++) {
    srtpMRBTableEntry* candidate = mrbTable->find(lookup);
    if (candidate == nullptr)
      break;
    addToUsedPool(lookup);
    if (candidate->correlatedAddr != 0) {
...
void srtp::outPrefetcherPGOInfo() {}
...
bool srtpMetaTable::insert(uint64_t key, const srtpMetaTableEntry& data)
{
  ...
  Entry victim_entry = Super::insert(key, data); // SetAssociativeCache<srtpMetaTableEntry>::insert(key, data);
