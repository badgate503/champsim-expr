...
int resize_kr::issue_metatable(resize_krMetaTable* metaTable, uint64_t pc, uint64_t lookup, std::vector<uint64_t>& addresses)
{
  int issued = 0;
  for (int i = 0; i < globalDegree; i++) {
    resize_krMetaTableEntry* candidate = metaTable->find(lookup);
    meta_table_lookups++;
...
uint32_t resize_kr::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type,
                                            uint32_t metadata_in, std::string latepf)
{
  ...
  resize_krMetaTableEntry* metadata = metaTable->find(lookup_key);
  if (metadata) {
  ...
  if (last_addr != 0 && last_addr != block_addr) {
    ...
    resize_krMetaTableEntry* last_meta = metaTable->find(insert_key);
    ...
    if (last_meta) {
      ...
      if (last_meta->correlated_addr == block_addr) {
        ...
        resize_krMetaTableEntry temp_entry(block_addr);
#ifdef NOMD_WHEN_HIT
        if (!cache_hit)
...
uint32_t resize_kr::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
  meta_table_prefetches.erase(evicted_addr.to<uint64_t>() >> LOG2_BLOCK_SIZE);
  return metadata_in;
}

void resize_kr::prefetcher_final_stats()
{
#ifdef MISS_CLASS_LOG
  logfile.close();
#endif
...
void resize_kr::prefetcher_late_prefetch(champsim::address addr, champsim::address ip, std::string where)
{
#ifdef MISS_CLASS_LOG
...
void resize_kr::prefetcher_cycle_operate() {}

bool resize_krMetaTable::insert(uint64_t key, const resize_krMetaTableEntry& data)
{
...
bool resize_krMetaTable::insert(uint64_t key, const resize_krMetaTableEntry& data, uint64_t rrpv_value)
{
  reverse_metatable[data.correlated_addr].insert(key);
  Entry victim_entry = Super::insert(key, data);
  // Super::set_mru(key);
