#include "infpctable.h"

#include <cassert>
#include <utility>

int infpctable::issue_metatable(infpctableMetaTable* metaTable, uint64_t pc, uint64_t lookup, std::vector<uint64_t>& addresses)
{
  int issued = 0;
  for (int i = 0; i < globalDegree; i++) {
    infpctableMetaTableEntry* candidate = metaTable->find(lookup);
    meta_table_lookups++;
...
uint32_t infpctable::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type,
  ...
  infpctableMetaTableEntry* metadata = metaTable->find(lookup_key);
  if (metadata) {
  ...
  if (last_addr != 0 && last_addr != block_addr) {
    ...
    infpctableMetaTableEntry* last_meta = metaTable->find(insert_key);
    ...
    if (last_meta) {
      ...
      if (last_meta->correlated_addr == block_addr) {
        ...
        infpctableMetaTableEntry temp_entry(block_addr);
#ifdef NOMD_WHEN_HIT
        if (!cache_hit)
...
uint32_t infpctable::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
...
void infpctable::prefetcher_final_stats()
{
#ifdef MISS_CLASS_LOG
  logfile.close();
#endif
...
void infpctable::prefetcher_late_prefetch(champsim::address addr, champsim::address ip, std::string where)
{
#ifdef MISS_CLASS_LOG
...
void infpctable::prefetcher_cycle_operate() {}

bool infpctableMetaTable::insert(uint64_t key, const infpctableMetaTableEntry& data)
{
