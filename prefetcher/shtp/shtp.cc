...
int shtp::issue_metatable(shtpMetaTable* metaTable, uint64_t lookup, std::vector<uint64_t>& addresses)
{
  int issued = 0;
  for (int i = 0; i < globalDegree; i++) {
    shtpMetaTableEntry* candidate = metaTable->find(lookup);
    meta_table_lookups++;
...
uint32_t shtp::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type,
                                            uint32_t metadata_in, std::string latepf)
{
  ...
  shtpMetaTableEntry* metadata = metaTable->find(lookup_key);
  if (metadata) {
  ...
  if (last_addr != 0 && last_addr != block_addr) {
    ...
    shtpMetaTableEntry* last_meta = metaTable->find(insert_key);

    if (last_meta) {
      bool matched = false;
      if (last_meta->correlated_addr == block_addr) {
        matched = true;
        metaTable->touch(insert_key, pc);
      } else {
        uint64_t victim_addr = last_meta->correlated_addr;
        shtpMetaTableEntry temp_entry(block_addr);
#ifdef NOMD_WHEN_HIT
        if (!cache_hit)
          metaTable->insert(insert_key, temp_entry, 1);
#else
        metaTable->insert(insert_key, temp_entry, pc);
#endif
      }
    } else {
      shtpMetaTableEntry temp_entry(block_addr);
      ...
      if (!cache_hit)
        if (!metaTable->insert(insert_key, temp_entry, 1))
          numEntriesinTable++;
#else
      if (!metaTable->insert(insert_key, temp_entry, pc)) {
        numEntriesinTable++;
      }
#endif
    }
...
uint32_t shtp::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
  meta_table_prefetches.erase(evicted_addr.to<uint64_t>() >> LOG2_BLOCK_SIZE);
  return metadata_in;
}

void shtp::prefetcher_final_stats()
{
#ifdef ELABORATE_LOG
  logfile.close();
#endif
...
void shtp::prefetcher_late_prefetch(champsim::address addr, champsim::address ip, std::string where)
{
#ifdef ELABORATE_LOG
...
void shtp::prefetcher_cycle_operate() {}

bool shtpMetaTable::insert(uint64_t key, const shtpMetaTableEntry& data, uint64_t pc)
{
