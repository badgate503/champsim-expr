...
struct shtpMetaTableEntry {

  uint64_t correlated_addr;
  bool used;

  shtpMetaTableEntry() : correlated_addr(0), used(false) {};
  shtpMetaTableEntry(uint64_t addr) : correlated_addr(addr) {};
};

class shtpMetaTable : public SHIPSetAssociativeCache<shtpMetaTableEntry>
{
  typedef SHIPSetAssociativeCache<shtpMetaTableEntry> Super;

public:
  std::unordered_map<uint64_t, std::set<uint64_t>> reverse_metatable;
  shtp* prefetcher;

  shtpMetaTable(int size, int num_ways) : Super(size, num_ways), prefetcher(nullptr) {}

  void setpp(shtp* p) { prefetcher = p; }

  shtpMetaTableEntry* find(uint64_t key)
  {
  ...
  */
  bool insert(uint64_t key, const shtpMetaTableEntry& data, uint64_t pc);

  Entry* erase(uint64_t key) { return Super::erase(key); }
};

class shtp : public champsim::modules::prefetcher
{
public:
  // BaseTags* cachetags;
  ...
  shtpMetaTable* metaTable = new shtpMetaTable(META_TABLE_SIZE, META_TABLE_ASSOC);
  ...
  int issue_metatable(shtpMetaTable* metaTable, uint64_t lookup, std::vector<uint64_t>& addresses);
...
#endif // __MEM_CACHE_PREFETCH_shtp_HH__
