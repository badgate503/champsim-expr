...
struct lfutpMetaTableEntry {

  uint64_t correlated_addr;
  bool used;

  lfutpMetaTableEntry() : correlated_addr(0), used(false) {};
  lfutpMetaTableEntry(uint64_t addr) : correlated_addr(addr) {};
};

class lfutpMetaTable : public LFUSetAssociativeCache<lfutpMetaTableEntry>
{
  typedef LFUSetAssociativeCache<lfutpMetaTableEntry> Super;

public:
  std::unordered_map<uint64_t, std::set<uint64_t>> reverse_metatable;
  lfutp* prefetcher;

  lfutpMetaTable(int size, int num_ways) : Super(size, num_ways), prefetcher(nullptr) {}

  void setpp(lfutp* p) { prefetcher = p; }

  lfutpMetaTableEntry* find(uint64_t key)
  {
  ...
  */
  bool insert(uint64_t key, const lfutpMetaTableEntry& data);

  Entry* erase(uint64_t key) { return Super::erase(key); }
};

class lfutp : public champsim::modules::prefetcher
{
public:
  // BaseTags* cachetags;
  ...
  lfutpMetaTable* metaTable = new lfutpMetaTable(META_TABLE_SIZE, META_TABLE_ASSOC);
  ...
  int issue_metatable(lfutpMetaTable* metaTable, uint64_t lookup, std::vector<uint64_t>& addresses);
...
#endif // __MEM_CACHE_PREFETCH_lfutp_HH__
