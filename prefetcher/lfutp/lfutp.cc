...
int lfutp::issue_metatable(lfutpMetaTable* metaTable, uint64_t lookup, std::vector<uint64_t>& addresses)
{
  int issued = 0;
  for (int i = 0; i < globalDegree; i++) {
    lfutpMetaTableEntry* candidate = metaTable->find(lookup);
    meta_table_lookups++;
...
uint32_t lfutp::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type,
                                            uint32_t metadata_in, std::string latepf)
{
  ...
  lfutpMetaTableEntry* metadata = metaTable->find(lookup_key);
  if (metadata) {
  ...
  if (last_addr != 0 && last_addr != block_addr) {
    ...
    lfutpMetaTableEntry* last_meta = metaTable->find(insert_key);
    ...
    if (last_meta) {
      ...
      if (last_meta->correlated_addr == block_addr) {
        ...
        lfutpMetaTableEntry temp_entry(block_addr);
#ifdef NOMD_WHEN_HIT
        if (!cache_hit)
...
uint32_t lfutp::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
  meta_table_prefetches.erase(evicted_addr.to<uint64_t>() >> LOG2_BLOCK_SIZE);
  return metadata_in;
}

void lfutp::prefetcher_final_stats()
{
#ifdef ELABORATE_LOG
  logfile.close();
#endif
...
void lfutp::prefetcher_late_prefetch(champsim::address addr, champsim::address ip, std::string where)
{
#ifdef ELABORATE_LOG
...
void lfutp::prefetcher_cycle_operate() {}

bool lfutpMetaTable::insert(uint64_t key, const lfutpMetaTableEntry& data)
{
