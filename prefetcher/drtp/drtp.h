...
struct drtpMetaTableEntry {

  uint64_t correlated_addr;
  bool used;

  drtpMetaTableEntry() : correlated_addr(0), used(false) {};
  drtpMetaTableEntry(uint64_t addr) : correlated_addr(addr) {};
};

class drtpMetaTable : public DRRIPSetAssociativeCache<drtpMetaTableEntry>
{
  typedef DRRIPSetAssociativeCache<drtpMetaTableEntry> Super;

public:
  std::unordered_map<uint64_t, std::set<uint64_t>> reverse_metatable;
  drtp* prefetcher;

  drtpMetaTable(int size, int num_ways) : Super(size, num_ways), prefetcher(nullptr) {}

  void setpp(drtp* p) { prefetcher = p; }

  drtpMetaTableEntry* find(uint64_t key)
  {
  ...
  */
  bool insert(uint64_t key, const drtpMetaTableEntry& data);

  Entry* erase(uint64_t key) { return Super::erase(key); }
};

class drtp : public champsim::modules::prefetcher
{
public:
  // BaseTags* cachetags;
  ...
  drtpMetaTable* metaTable = new drtpMetaTable(META_TABLE_SIZE, META_TABLE_ASSOC);
  ...
  int issue_metatable(drtpMetaTable* metaTable, uint64_t lookup, std::vector<uint64_t>& addresses);
...
#endif // __MEM_CACHE_PREFETCH_drtp_HH__
