...
public:
  ...
  // set llc connection
  void impl_setup_prefetcher_llc_connection(CACHE* llc_cache) const;
...
private:
  ...
  std::deque<std::pair<uint64_t, tag_lookup_type>> delay_prefetch_queue{};
...
public:
  ...
  void set_available_ways(uint32_t ways) {
    available_ways = std::min(ways, NUM_WAY);
  }
  
  uint32_t get_available_ways() const {
...
#include "module_decl.inc"
  ...
  struct prefetcher_module_concept {
    ...
    virtual uint32_t impl_prefetcher_cache_fill(champsim::address addr, long set, long way, bool prefetch, champsim::address evicted_addr,
                                                uint32_t metadata_in, champsim::address ip) = 0;
    virtual void impl_prefetcher_late_prefetch(champsim::address addr, champsim::address ip, std::string where) = 0;
    ...
    virtual void impl_setup_prefetcher_llc_connection(CACHE* llc_cache) = 0;
  };
  ...
  struct prefetcher_module_model final : prefetcher_module_concept {
    ...
    [[nodiscard]] uint32_t impl_prefetcher_cache_fill(champsim::address addr, long set, long way, bool prefetch, champsim::address evicted_addr,
                                                      uint32_t metadata_in, champsim::address ip) final;
    void impl_prefetcher_late_prefetch(champsim::address addr, champsim::address ip, std::string where) final;
    ...
    void impl_setup_prefetcher_llc_connection(CACHE* llc_cache) final {
        [[maybe_unused]] auto process_one = [&](auto& p) {
            using namespace champsim::modules;
            //detect whether need to connect llc or not
            if constexpr (prefetcher::has_set_llc_reference<decltype(p), CACHE*>) {
                p.set_llc_reference(llc_cache);
            }
            // else if constexpr (std::is_same_v<std::decay_t<decltype(p)>, other_prefetcher>) {
            //     p.set_llc_reference(llc_cache);
            // }
  ...
  [[nodiscard]] uint32_t impl_prefetcher_cache_fill(champsim::address addr, long set, long way, bool prefetch, champsim::address evicted_addr,
                                                    uint32_t metadata_in, champsim::address ip) const;
  void impl_prefetcher_late_prefetch(champsim::address addr, champsim::address ip, std::string where) const;
  ...
  explicit CACHE(champsim::cache_builder<champsim::cache_builder_module_type_holder<Ps...>, champsim::cache_builder_module_type_holder<Rs...>> b)
      : champsim::operable(b.m_clock_period), upper_levels(b.m_uls), lower_level(b.m_ll), lower_translate(b.m_lt), NAME(b.m_name), NUM_SET(b.get_num_sets()),
        NUM_WAY(b.get_num_ways()), available_ways(b.get_num_ways()), MSHR_SIZE(b.get_num_mshrs()), PQ_SIZE(b.m_pq_size), HIT_LATENCY(b.get_hit_latency() * b.m_clock_period),
...
uint32_t CACHE::prefetcher_module_model<Ps...>::impl_prefetcher_cache_operate(champsim::address addr, champsim::address ip, bool cache_hit,
                                                                              bool useful_prefetch, access_type type, uint32_t metadata_in, std::string latepf )
{
  ...
  [[maybe_unused]] auto process_one = [&](auto& p) {
    ...
    if constexpr (prefetcher::has_cache_operate<decltype(p), champsim::address, champsim::address, bool, bool, access_type, uint32_t, std::string>)
      return return_type{p.prefetcher_cache_operate(addr, ip, cache_hit, useful_prefetch, type, metadata_in, latepf)};
...
uint32_t CACHE::prefetcher_module_model<Ps...>::impl_prefetcher_cache_fill(champsim::address addr, long set, long way, bool prefetch,
                                                                           champsim::address evicted_addr, uint32_t metadata_in, champsim::address ip)
{
  using return_type = uint32_t;
  [[maybe_unused]] auto process_one = [&](auto& p) {
    using namespace champsim::modules;
    ...
    if constexpr (prefetcher::has_cache_fill<decltype(p), champsim::address, long, long, bool, champsim::address, uint32_t, champsim::address>)
      return return_type{p.prefetcher_cache_fill(addr, set, way, prefetch, evicted_addr, metadata_in, ip)};
...
template <typename... Ps>
void CACHE::prefetcher_module_model<Ps...>::impl_prefetcher_late_prefetch(champsim::address addr, champsim::address ip, std::string where)
{ 
  [[maybe_unused]] auto process_one = [&](auto& p) {
    using namespace champsim::modules;
    if constexpr (prefetcher::has_late_prefetch<decltype(p), champsim::address, champsim::address, std::string>)
      p.prefetcher_late_prefetch(addr, ip, where);
  };
  std::apply([&](auto&... p) { (..., process_one(p)); }, intern_);
}
