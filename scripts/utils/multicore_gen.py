...
parser.add_argument("--core", "-c", type=int, default=2, help="Number of cores of the sampled combinations")
parser.add_argument("--num-per-suite", "-n", type=int, default=2, help="Number of combinations to sample")
parser.add_argument("--no-repeat", action="store_true", help="Disallow repeated traces in a sampled combinations (default: allow repeats)")
...
def load_trace_map(path: str) -> Dict[str, List[str]]:
...
def main() -> None:
    tracelist_path = cur_path / "tracelist"
    trace_map = load_trace_map(tracelist_path)
    ...
    with open(cur_path / output_file, "w") as f:
    ...
    mode_str = "Allowing repeats" if ALLOW_REPEAT else "Disallowing repeats"
    print(f"Task completed | Mode: {mode_str} | Cores: {args.core} | File: {output_file}")
