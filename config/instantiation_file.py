...
def get_instantiation_lines(cores, caches, ptws, pmem, vmem, build_id):
    ...
    yield '  // Setup Triangle prefetcher LLC connection'
    yield '  for (auto& cache : caches) {'
    yield '    if (cache.NAME.find("L2C") != std::string::npos) {'
    yield '      // Find LLC cache'
    yield '      auto llc_it = std::find_if(std::begin(caches), std::end(caches),'
    yield '                                 [](const auto& c) { return c.NAME == "LLC"; });'
    yield '      if (llc_it != std::end(caches)) {'
    yield '        // Setup prefetcher connection if present'
    yield '        cache.impl_setup_prefetcher_llc_connection(&(*llc_it));'
    yield '      }'
    yield '    }'
    yield '  }'
